...
export default function RepairProgress({ status }: any) {
  return (
    <div className="mt-5">
      <h4 className="sr-only">{status_repair[status].label}</h4>
      <p className="text-sm font-medium text-gray-900">{status_repair[status].text}</p>
      <div className="mt-2" aria-hidden="true">
        <div className="bg-gray-200 rounded-full overflow-hidden">
          <div
            className="h-2 bg-accent rounded-full"
            style={{ width: `${(status_repair[status].value * 100) / status_repair.total}%` }}
          />
        </div>
