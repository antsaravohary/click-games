...
export default function CreateOrUpdateCategoriesForm({
  ...
  const {
    ...
    defaultValues: initialValues
      ? {
          ...initialValues,
          champs_required: initialValues?.champs_required?.map((c) =>{
            const champ=champs_required_data.find((champ) => champ.value === c);
            return {label:champ?.label,value:champ?.value}
          }
  ...
  const champs_required = watch("champs_required");
  ...
  const onSubmit = async (values: FormValues) => {
    const input = {
      ...
      types: values?.types.map((t: { id: any }) => t.id),
      type_label:values?.type_label,
      champs_required: values?.champs_required?.map((c) => c.value),
        ...
        <Card className="w-full sm:w-8/12 md:w-2/3">
          ...
          {!parent ? (
            ...
            <div className="mt-5 mb-5">
              <Label>{t("form:input-label-champ_required")}</Label>
              <SelectInput
                isMulti
                name="champs_required"
                control={control}
                options={champs_required_data?.map((champ)=>({label:champ.label,value:champ.value}))}
                isClearable={true}
              />
            </div>
          )}
          {champs_required?.some((t) => t.value === "type") && (
            <div className="border p-2">
            <h4 className="text-md font-semibold my-2">Options types</h4>
            <Input
              
              label={t("form:input-label-type-name")}
              {...register("type_label")}
              error={t(errors.type_label?.message!)}
              variant="outline"
              className="mb-2"
            />
